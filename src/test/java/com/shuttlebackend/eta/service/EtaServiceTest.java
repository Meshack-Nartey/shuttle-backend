...
class EtaServiceTest {
    ...
    void telemetryBasedEta() {
        ...
        when(telemetryRepository.findTopByVehicleIdOrderByRecordedAtDesc(vid))
                .thenReturn(Optional.of(last));
        when(telemetryRepository.findTop5ByVehicleIdOrderByRecordedAtDesc(vid))
                .thenReturn(Collections.singletonList(last));

        List<Waypoint> waypoints = Arrays.asList(
                new Waypoint(1.001, 1.001),
                new Waypoint(1.002, 1.002)
        );
        ...
        // FIXED AMBIGUOUS METHOD ISSUE
        ArgumentCaptor<String> topicCaptor = ArgumentCaptor.forClass(String.class);

        verify(messagingTemplate, times(1))
                .convertAndSend(topicCaptor.capture(), any(Object.class));

        assertThat(topicCaptor.getValue())
                .contains("/topic/vehicles/" + vid + "/eta");
    }
    ...
    void averageSpeedEstimationUsed() {
        ...
        when(telemetryRepository.findTop5ByVehicleIdOrderByRecordedAtDesc(vid))
                .thenReturn(Arrays.asList(last, prev1, prev2));
    ...
    void unavailableWhenNoTelemetryOrRoute() {
        ...
        when(routeService.getRemainingWaypoints(vid))
